...
class EmbeddingService:
    ...
    async def embed_texts(
        ...
        try:
            ...
            # Safety fallback: if workspace is configured with an OpenAI embedding model
            # incompatible with the vector column dimension, switch to local embeddings.
            # This keeps chat usable with vector(384) databases and avoids hard 500 errors.
            if not use_local_flag:
                logger.warning(
                    "Embedding dimension mismatch for model=%s with vector_dimension=%s. "
                    "Falling back to local embeddings model=%s. Original error: %s",
                    model,
