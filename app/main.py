...
from app.core.site_log_forwarder import report_to_laravel
...
async def unhandled_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    report_to_laravel(
        message=exc_message or exc_type.__name__,
        severity="critical" if is_critical else "error",
        source="python",
        category=exc_type.__name__,
        exception_class=f"{exc_type.__module__}.{exc_type.__name__}" if getattr(exc_type, "__module__", None) else exc_type.__name__,
        stack_trace=exc_traceback_str,
        context=context,
        status_code=500,
        request_method=request.method,
        request_path=request.url.path,
